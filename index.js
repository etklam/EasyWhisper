...
import { access, mkdir, readdir, rm, stat, readFile, writeFile, rename, unlink } from "node:fs/promises";
import fs, { constants as constants$1, createWriteStream } from "node:fs";
...
const OllamaClient = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  chat,
  checkOllama,
  listModels
...
const DEFAULT_PROMPTS = {
  translate: (text, targetLang) => `Translate the following text into ${targetLang}. Output only the translation, no explanation:

${text}`,
  summary: (text) => `Provide a concise summary of the following transcript. List key points:

${text}`,
  correct: (text, lang) => `The following is ${lang} text from speech recognition. Fix typos and recognition errors, preserve the original meaning, output only the corrected text:

${text}`
};
function getPrompt(task, customPrompts) {
...
function createCustomPrompt(task, template) {
  switch (task) {
    case "summary":
      return ((text) => applyPromptTemplate(template, text));
    case "translate":
      return ((text, targetLang) => applyPromptTemplate(template, text, targetLang));
    case "correct":
      return ((text, lang) => applyPromptTemplate(template, text, lang));
  }
}
function applyPromptTemplate(template, text, secondArg) {
  let result = template;
  const replacements = {
    "{text}": text,
...
class AiPipeline {
  ...
  activeCount = 0;
  async init() {
    const { checkOllama: checkOllama2 } = await Promise.resolve().then(() => OllamaClient);
    const running = await checkOllama2();
    if (!running) {
  ...
  async processTask(task) {
    ...
    const effectiveChunkSize = task.batchMode === false ? task.chunkSize ?? this.options.chunkSize : Math.min(task.chunkSize ?? this.options.chunkSize, availableContextWindow);
  ...
  createProgress(task, index, totalChunks) {
    return {
      taskId: task.id,
      taskType: task.taskType,
  ...
  buildPrompt(task, promptFn, context, text) {
  ...
  buildContext(chunks, currentIndex, taskType) {
...
`;
  ...
  splitParagraphBySentences(paragraph, chunkSize) {
    const chunks = [];
    let current = "";
    let currentTokens = 0;
  ...
  splitOversizedSentence(sentence, chunkSize) {
    const pieces = [];
  ...
  async runWithTimeout(runner, timeoutMs, upstreamSignal) {
    let timeoutHandle;
...
function mergeAbortSignals(...signals) {
  const availableSignals = signals.filter((signal) => signal !== void 0);
  ...
  if (abortedSignal) {
    ...
    controller2.abort(abortedSignal.reason);
...
function normalizeAbortError(reason, cancelledByUser) {
...
function findBestBreakIndex(text, preferredIndex) {
...
const activeTaskControllers = /* @__PURE__ */ new Map();
function createPipeline(model) {
...
function registerAiIpc(_mainWindow) {
  if (typeof ipcMain$1.removeHandler === "function") {
    ipcMain$1.removeHandler(IPC_CHANNELS.AI_RUN);
  }
  ipcMain$1.handle(IPC_CHANNELS.AI_RUN, async (_event, payload) => {
  ...
  if (typeof ipcMain$1.removeHandler === "function") {
    ipcMain$1.removeHandler(IPC_CHANNELS.AI_STOP);
  }
  ipcMain$1.handle(IPC_CHANNELS.AI_STOP, async (_event, payload) => {
  ...
  if (typeof ipcMain$1.removeHandler === "function") {
    ipcMain$1.removeHandler(IPC_CHANNELS.AI_GET_STATUS);
  }
  ipcMain$1.handle(IPC_CHANNELS.AI_GET_STATUS, async () => {
  ...
  if (typeof ipcMain$1.removeHandler === "function") {
    ipcMain$1.removeHandler(IPC_CHANNELS.AI_LIST_MODELS);
  }
  ipcMain$1.handle(IPC_CHANNELS.AI_LIST_MODELS, async () => {
    const connected = await checkOllama().catch(() => false);
    if (!connected) {
      return [];
    }
...
const SUPPORTED_AUDIO_FORMATS = ["mp3", "wav", "m4a", "flac"];
const SUPPORTED_VIDEO_FORMATS = ["mp4", "mov", "mkv", "avi"];
const SUPPORTED_OUTPUT_FORMATS = ["txt", "srt", "vtt", "json"];
const VALID_OUTPUT_FORMATS = SUPPORTED_OUTPUT_FORMATS;
...
class AudioProcessor {
  ...
  async convertToWav(inputPath, outputPath, onProgress) {
    ...
    if (!this.needsConversion(inputPath) && (outputPath === void 0 || inputPath === resolvedOutputPath)) {
      return inputPath;
    }
    ...
    return new Promise((resolve2, reject) => {
      ...
      proc.stderr.on("data", (chunk) => parseLine.push(chunk));
      ...
      proc.on("close", (code2) => {
        if (code2 === 0) {
          ...
          resolve2(resolvedOutputPath);
          return;
        }
        const detail = stderrLines.slice(-3).join(" | ");
        reject(new Error(`ffmpeg exited with code ${code2}${detail ? `: ${detail}` : ""}`));
      });
    });
  }
  isAudioFile(filename) {
    return SUPPORTED_AUDIO_FORMATS.includes(getExtension(filename));
  }
  isVideoFile(filename) {
    return SUPPORTED_VIDEO_FORMATS.includes(getExtension(filename));
  }
...
function createAudioProcessor() {
  ...
  const cacheDir = path.join(app$1.getPath("temp"), "fosswhisper-audio");
  return new AudioProcessor(ffmpegPath, cacheDir);
}
function registerAudioHandlers(mainWindow) {
  const audioProcessor = createAudioProcessor();
  if (typeof ipcMain$1.removeHandler === "function") {
    ipcMain$1.removeHandler(IPC_CHANNELS.AUDIO_CONVERT);
  }
  ipcMain$1.handle(
...
const SUPPORTED_WHISPER_MODELS = WHISPER_MODEL_IDS;
class WhisperMac {
  projectRoot;
  modelsDir;
  whisperDir;
    ...
    return new Promise((resolve2, reject) => {
      ...
      proc.on("close", async (code2) => {
        if (code2 !== 0) {
          ...
          reject(new Error(`whisper-cli exited with code ${code2}${detail ? `: ${detail}` : ""}`));
          return;
        }
        try {
          options.onProgress({
  ...
  async listModels() {
    ...
    return Promise.all(
      SUPPORTED_WHISPER_MODELS.map(async (modelId) => {
        ...
        const metadata2 = MODEL_CATALOG[modelId];
        try {
  ...
  async downloadModel(modelId, onProgress) {
    ...
    try {
      await access(targetPath, constants$1.R_OK);
      return targetPath;
    } catch {
  ...
  async resolveWhisperCliPath() {
    const candidates = [
      ...
      process.resourcesPath ? path.resolve(process.resourcesPath, "whisper-cli") : void 0
  ...
  async resolveModelPath(modelPathOrId) {
    const normalizedPath = path.resolve(modelPathOrId);
    try {
      await access(normalizedPath, constants$1.R_OK);
      return normalizedPath;
    } catch {
      const modelId = path.basename(modelPathOrId);
  ...
  buildArgs(options, outputPath, modelPath) {
  ...
  async readTranscriptionText(outputPath) {
    const raw = await readFile(outputPath, "utf8");
    const json = JSON.parse(raw);
    return json.text ?? "";
  }
  async mockTranscribe(options, outputPath, startedAt) {
    options.onProgress({
      taskId: options.taskId,
      progress: 0,
      stage: "preparing",
    ...
    const request = (currentUrl) => {
      const req = get(currentUrl, (response) => {
        ...
        if ([301, 302, 307, 308].includes(statusCode) && response.headers.location) {
          response.resume();
          request(response.headers.location.startsWith("http") ? response.headers.location : new URL(response.headers.location, currentUrl).toString());
          return;
        }
        if (statusCode !== 200) {
          response.resume();
        ...
        const totalBytes = totalBytesHeader ? Number(totalBytesHeader) : void 0;
        let receivedBytes = 0;
...
const whisperMac$1 = new WhisperMac();
function registerModelHandlers(mainWindow) {
  if (typeof ipcMain$1.removeHandler === "function") {
    ipcMain$1.removeHandler(IPC_CHANNELS.MODEL_LIST);
    ipcMain$1.removeHandler(IPC_CHANNELS.MODEL_DOWNLOAD);
  }
  ipcMain$1.handle(IPC_CHANNELS.MODEL_LIST, async () => {
    return whisperMac$1.listModels();
  });
  ipcMain$1.handle(
    IPC_CHANNELS.MODEL_DOWNLOAD,
    async (_event, payload) => {
      const modelPath = await whisperMac$1.downloadModel(payload.modelId, (progress) => {
...
class OutputFormatter {
  ...
  formatAsVtt(result) {
    ...
    const body = result.segments.map((segment) => {
      return [
...
const SUPPORTED_FORMATS = ["txt", "srt", "vtt", "json"];
function registerOutputHandlers() {
  if (typeof ipcMain$1.removeHandler === "function") {
    ipcMain$1.removeHandler(IPC_CHANNELS.OUTPUT_FORMAT);
    ipcMain$1.removeHandler(IPC_CHANNELS.OUTPUT_GET_FORMATS);
  }
  ipcMain$1.handle(
    ...
    async (_event, payload) => {
      ...
      const nextOutputPath = payload.format === "json" ? payload.outputPath : payload.outputPath.replace(/\.json$/i, extension);
  ...
  ipcMain$1.handle(IPC_CHANNELS.OUTPUT_GET_FORMATS, async () => {
...
const VALID_LOCALES = ["en", "zh-TW", "zh-CN"];
...
class SettingsManager {
  ...
  constructor() {
    ...
    this.queueStore = new ElectronStore({
      name: "queue",
      defaults: { items: [] }
    });
    this.settings = {
      ...DEFAULT_SETTINGS,
      ...this.readStoreValue(this.settingsStore, void 0) ?? {}
    };
    this.queueItems = [...this.readStoreValue(this.queueStore, "items") ?? []];
  }
  getSettings() {
  ...
  saveQueueItems(items2) {
    this.queueItems = items2.map((item) => ({ ...item }));
    this.scheduleQueuePersist();
  }
  updateQueueItemStatus(id2, status) {
    const index = this.queueItems.findIndex((candidate) => candidate.id === id2);
    if (index < 0) {
      return;
    }
  ...
  removeQueueItem(id2) {
    this.queueItems = this.queueItems.filter((item) => item.id !== id2);
    this.scheduleQueuePersist();
  }
  clearQueue() {
    this.queueItems = [];
  ...
  importSettings(data) {
    if (!data || typeof data !== "object" || !("settings" in data) || typeof data.settings !== "object") {
  ...
  validateSettings(settings) {
    ...
    for (const format2 of settings.outputFormats) {
      if (!isValidOutputFormat(format2)) {
        throw new Error(`Invalid output format: ${format2}`);
      }
    }
  }
  ...
  readStoreValue(store, key) {
    ...
    if (typeof key === "string" && "store" in store) {
      const raw = store.store;
      return raw?.[key];
    }
    return void 0;
  }
  writeStoreValue(store, key, value) {
    if (!store || typeof store !== "object") {
      return;
    }
  ...
  tryInvoke(store, method) {
    if (store && typeof store === "object" && typeof store[method] === "function") {
      store.reset();
      return;
    }
    if (method === "reset") {
...
function isValidOutputFormat(format2) {
  return VALID_OUTPUT_FORMATS.includes(format2);
}
const settingsManager = new SettingsManager();
function registerSettingsIpc() {
  if (typeof ipcMain$1.removeHandler === "function") {
    ipcMain$1.removeHandler(IPC_CHANNELS.SETTINGS_GET);
  }
  ipcMain$1.handle(IPC_CHANNELS.SETTINGS_GET, async () => {
    return toWorkflowSettings(settingsManager.getSettings());
  });
  if (typeof ipcMain$1.removeHandler === "function") {
    ipcMain$1.removeHandler(IPC_CHANNELS.SETTINGS_SET);
  }
  ipcMain$1.handle(
...
function toWorkflowSettings(settings) {
  return {
...
function toSettingsPatch(partial) {
  const next2 = {};
  if (partial.modelPath !== void 0) next2.whisperModel = partial.modelPath;
  if (partial.threads !== void 0) next2.whisperThreads = partial.threads;
  if (partial.language !== void 0) next2.whisperLanguage = partial.language;
  if (partial.useMetal !== void 0) next2.whisperUseMetal = partial.useMetal;
  if (partial.outputDir !== void 0) next2.outputDir = partial.outputDir;
  if (partial.outputFormats !== void 0) next2.outputFormats = partial.outputFormats;
  if (partial.ytdlpAudioFormat !== void 0) next2.ytdlpAudioFormat = partial.ytdlpAudioFormat;
  if (partial.ytdlpCookiesPath !== void 0) next2.ytdlpCookiesPath = partial.ytdlpCookiesPath || void 0;
  const aiPatch = {};
  const aiTaskPatch = {};
  if (partial.aiEnabled !== void 0) aiPatch.enabled = partial.aiEnabled;
  if (partial.aiModel !== void 0) aiPatch.model = partial.aiModel;
  if (partial.aiTargetLang !== void 0) aiPatch.targetLang = partial.aiTargetLang;
  if (partial.aiCustomPrompts !== void 0) aiPatch.customPrompts = partial.aiCustomPrompts;
  if (partial.aiCorrect !== void 0) aiTaskPatch.correct = partial.aiCorrect;
  if (partial.aiTranslate !== void 0) aiTaskPatch.translate = partial.aiTranslate;
  if (partial.aiSummary !== void 0) aiTaskPatch.summary = partial.aiSummary;
...
function registerWhisperIpc(mainWindow) {
  if (typeof ipcMain$1.removeHandler === "function") {
    ipcMain$1.removeHandler(IPC_CHANNELS.WHISPER_START);
  }
  ipcMain$1.handle(
    IPC_CHANNELS.WHISPER_START,
    async (_event, payload) => {
      const taskId = payload.taskId ?? randomUUID();
      void whisperMac.transcribe({
        ...payload,
        taskId,
...
class YtDlpDownloader {
  constructor(ytDlpPath, tmpDir) {
    this.ytDlpPath = ytDlpPath;
    this.tmpDir = tmpDir;
  }
  activeDownloads = /* @__PURE__ */ new Map();
  async downloadAudio(taskId, url, options = {}) {
    ...
    await new Promise((resolve2, reject) => {
      const proc = spawn$1(this.ytDlpPath, args);
      ...
      proc.stdout.on("data", (chunk) => parseLine.push(chunk));
      ...
      proc.on("close", (code2) => {
        ...
        const baseError = new Error(`yt-dlp exited with code ${code2}`);
...
function createDownloader() {
  ...
  const tmpDir = path.join(app$1.getPath("temp"), "fosswhisper-ytdlp");
  return new YtDlpDownloader(ytDlpPath, tmpDir);
}
function registerYtDlpHandlers(mainWindow) {
  const downloader = createDownloader();
  if (typeof ipcMain$1.removeHandler === "function") {
    ipcMain$1.removeHandler(IPC_CHANNELS.YTDLP_DOWNLOAD);
    ipcMain$1.removeHandler(IPC_CHANNELS.YTDLP_CANCEL);
  }
  ipcMain$1.handle(
    IPC_CHANNELS.YTDLP_DOWNLOAD,
    async (_event, payload) => {
      const taskId = payload.taskId ?? randomUUID();
      void downloader.downloadAudio(taskId, payload.url, {
