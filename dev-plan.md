...
- [x] Windows Phase 4 MVP 已完成並合併到 `main`
- [x] Windows active runtime 已從 `Const-me` / `whisper.dll` 改為 `whisper.cpp` Vulkan `whisper-cli.exe`
- [x] 合併後已在 `main` 重新驗證 `pnpm test:run`
- [x] 已在 Windows 本機成功產出 NSIS 安裝檔（`.exe`）
- [x] Desktop app icon source、generated assets、packaging wiring 已完成並通過 Windows 打包驗證
- [ ] 將本地已合併的 `main` push 到遠端
- [ ] 完成 Phase 5 剩餘發佈收尾：正式 release 流程、Mac notarization、auto-update
- [ ] 完成 Phase 4.6 後續擴展：CUDA、CPU fallback、多 binary 偵測、進一步抽共用層

> 註：Phase 1–3 的功能已在目前主線存在，但本輪工作重點是 Windows Phase 4 migration，尚未重新逐條 audit 早期 phase 的每一項驗收項目。
...
**目前狀態（2026-03-23）**：已完成。AMD GPU / Vulkan 路線已實機驗證，`Const-me` integration 已由 `whisper.cpp` CLI 取代，變更已合併到 `main`。
...
**目前狀態（2026-03-23）**：進行中。Windows 本地打包已驗證可產出 installer，但正式發佈流程、Mac notarization 與 auto-update 尚未完成。
